...
<div class="">
    ...
    <div class="table-responsive">
        <table  id="Contactsujet" class="table">
            <thead>
                <tr>
                    <th scope="col">N#</th>
            ...
            <tbody>
                @foreach($contactsujets as $contactsujet)
						<tr><td>{{ $contactsujet->id }}</td>
							<td>{{ $contactsujet->sujet }}</td>
						<td>
                    <a href="{{ route('admin.contactsujet.show', ['id' => $contactsujet->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.contactsujet.edit', ['id' => $contactsujet->id]) }}" class="btn btn-success btn-sm">
        ...
        <div class="d-flex justify-content-center">
            {{ $contactsujets->links('pagination::bootstrap-5') }}
        </div>
    </div>
</div>
</div>
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/contactsujets/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/contactsujets/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const tableHeaders = document.querySelectorAll('#Contactsujet th');
            ...
            tableHeaders.forEach(function(header, index) {
                ...
                const savedSelection = localStorage.getItem('selectedColumns#Contactsujet');
        ...
        function toggleColumn(columnIndex, show) {
            const dataTable = document.getElementById('Contactsujet');
        ...
        function saveSelection() {
            ...
            localStorage.setItem('selectedColumns#Contactsujet', JSON.stringify(selectedColumns));
        }
        ...
        function sortTable(columnIndex) {
            const table = document.getElementById('Contactsujet');
