            ...
            <thead>
                <tr>
                    <th scope="col">N#</th>
						...
						<th scope="col">Title6</th>
						<th scope="col">Title7</th>
						<th scope="col">Title8</th>
						<th scope="col">Title9</th>
						<th scope="col">Title10</th>
            ...
            <tbody>
                @foreach($titles as $title)
						<tr><td>{{ $title->id }}</td>
							...
							<td>{{ $title->title10 }}</td>
						<td>
                    <a href="{{ route('admin.title.show', ['id' => $title->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.title.edit', ['id' => $title->id]) }}" class="btn btn-success btn-sm">
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/titles/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/titles/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const tableHeaders = document.querySelectorAll('#Title th');
            ...
            tableHeaders.forEach(function(header, index) {
                ...
                const savedSelection = localStorage.getItem('selectedColumns#Title');
        ...
        function toggleColumn(columnIndex, show) {
            const dataTable = document.getElementById('Title');
        ...
        function saveSelection() {
            ...
            localStorage.setItem('selectedColumns#Title', JSON.stringify(selectedColumns));
        }
