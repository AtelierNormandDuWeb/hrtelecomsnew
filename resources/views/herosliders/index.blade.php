...
<div class="">
    ...
    <div class="table-responsive">
        <table  id="Heroslider" class="table">
            <thead>
                <tr>
                    <th scope="col">N#</th>
            ...
            <tbody>
                @foreach($herosliders as $heroslider)
						...
						<td>
                    <a href="{{ route('admin.heroslider.show', ['id' => $heroslider->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.heroslider.edit', ['id' => $heroslider->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $heroslider->id }}" class="btn btn-danger btn-sm deleteBtn">
        ...
        <div class="d-flex justify-content-center">
            {{ $herosliders->links('pagination::bootstrap-5') }}
        </div>
    </div>
</div>
</div>
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/herosliders/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/herosliders/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const tableHeaders = document.querySelectorAll('#Heroslider th');
            ...
            tableHeaders.forEach(function(header, index) {
                ...
                const savedSelection = localStorage.getItem('selectedColumns#Heroslider');
        ...
        function toggleColumn(columnIndex, show) {
            const dataTable = document.getElementById('Heroslider');
        ...
        function saveSelection() {
            ...
            localStorage.setItem('selectedColumns#Heroslider', JSON.stringify(selectedColumns));
        }
        ...
        function sortTable(columnIndex) {
            const table = document.getElementById('Heroslider');
