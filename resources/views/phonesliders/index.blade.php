...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.phoneslider.create') }}" class="btn btn-success m-1">
...
<div class="">
    ...
    <div class="table-responsive">
        <table  id="Phoneslider" class="table">
            <thead>
                <tr>
                    <th scope="col">N#</th>
            ...
            <tbody>
                @foreach($phonesliders as $phoneslider)
						...
						<td>
                    <a href="{{ route('admin.phoneslider.show', ['id' => $phoneslider->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.phoneslider.edit', ['id' => $phoneslider->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $phoneslider->id }}" class="btn btn-danger btn-sm deleteBtn">
        ...
        <div class="d-flex justify-content-center">
            {{ $phonesliders->links('pagination::bootstrap-5') }}
        </div>
    </div>
</div>
</div>
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/phonesliders/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/phonesliders/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const tableHeaders = document.querySelectorAll('#Phoneslider th');
            ...
            tableHeaders.forEach(function(header, index) {
                ...
                const savedSelection = localStorage.getItem('selectedColumns#Phoneslider');
        ...
        function toggleColumn(columnIndex, show) {
            const dataTable = document.getElementById('Phoneslider');
        ...
        function saveSelection() {
            ...
            localStorage.setItem('selectedColumns#Phoneslider', JSON.stringify(selectedColumns));
        }
        ...
        function sortTable(columnIndex) {
            const table = document.getElementById('Phoneslider');
